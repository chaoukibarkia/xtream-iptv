            ...
            public void onClick(View view) {
              ...
              public void run() {
                ...
                backmenuelement="compteBtn";
                  // Use new Xtream API for account info
                  client.get(MainActivity.mainlink + "/player_api.php?username=" + user + "&password=" + pass, new JsonHttpResponseHandler() {
                      ...
                      public void onSuccess(int statusCode, Header[] headers, JSONObject res) {
                          if (res != null) {
                              ...
                              try {
                                  JSONObject user_info = res.getJSONObject("user_info");
                                  String activecode = sharedPref.getString("activecode", "");
                                  
                                  accountname.setText(activecode);
                                  
                                  // Convert Unix timestamp to readable date
                                  String createdAt = user_info.has("created_at") ? 
                                      new java.text.SimpleDateFormat("dd/MM/yyyy").format(
                                          new java.util.Date(user_info.getLong("created_at") * 1000L)) : "N/A";
                                  accountactivation.setText("Activé: " + createdAt);
                                  
                                  // Convert Unix timestamp to readable date
                                  String expiresAt = user_info.has("exp_date") && !user_info.isNull("exp_date") ? 
                                      new java.text.SimpleDateFormat("dd/MM/yyyy").format(
                                          new java.util.Date(Long.parseLong(user_info.getString("exp_date")) * 1000L)) : "Jamais";
                                  accountexpiration.setText("Expire: " + expiresAt);
                                  ...
                                  e.printStackTrace();
                                  Log.e("ERROR", "Failed to parse account info: " + e.getMessage());
                              }
                          }
                      }
                      ...
                      public void onFailure(int statusCode, Header[] headers, String res, Throwable t) {
                          Log.e("error", "Failed to fetch account info: " + t.getMessage());
                      }
                  });
